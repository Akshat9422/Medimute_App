...
const Video = ({ stream, isLocalStream, isDoctor, chosenChatDetails }) => {
  ...
  useEffect(() => {
    const handlePredictedClass = (predictedClass) => {
      setPredictedClass(predictedClass);
    };

    // Call updateOnTheRemoteSide and pass the callback function
    updateOnTheRemoteSide({ receiverUserId: chosenChatDetails.id, PredictedClass: null }, handlePredictedClass); // Pass PredictedClass as null initially
    ...
    const predict = async () => {
      ...
      if (hands.length > 0) {
        ...
        if(model2){
          ...
          // setPredictedClass(predictedClassIndex);
  ...
  return (
    <MainContainer>
      ...
      {predictedClass !== null && (
        <h2>{`Predicted Class NG: ${predictedClass}`}</h2>
      )}
    </MainContainer>
  );
};
